...
export default function ChatApp() {
  const [messages, setMessages] = useState([
    { sender: "Neha", text: "Local working on OS but At web , chating available OS is not working on web", audio: "" },
  ]);
  const [input, setInput] = useState("");
  const [audioSrc, setAudioSrc] = useState("");
  const [isListening, setIsListening] = useState(false);
  ...
  const recognitionRef = useRef(null);
  const isProcessingRef = useRef(false);
  const shouldResumeListeningAfterReply = useRef(false);
  ...
  const sendMessage = async (message) => {
    ...
    shouldResumeListeningAfterReply.current = true;
  ...
  return (
 <div className="flex flex-col h-screen bg-gradient-to-b from-gray-950 to-black text-cyan-200 font-mono p-3 sm:p-6">
      {/* Header */}
      <h1 className="text-xl sm:text-3xl font-extrabold text-center text-cyan-400 drop-shadow-lg bg-black bg-opacity-70 px-4 py-2 rounded-xl w-max mx-auto tracking-widest border border-cyan-600 shadow-cyan-500/50">
        🤖 AI Assistant
      </h1>

      {/* Chat Window */}
      <div
        className="flex-1 overflow-y-auto p-4 sm:p-6 mt-4 bg-gray-900/70 rounded-2xl shadow-xl mx-2 sm:mx-12 backdrop-blur-md border border-cyan-800 relative"
        style={{
          backgroundImage: "url('https://cdn.wallpapersafari.com/47/41/eQnFVK.jpg')",
          backgroundSize: "cover",
          backgroundPosition: "center",
        }}
      >
        {/* AI Robot Image Floating */}
        <div className="absolute top-3 right-3 w-16 h-16 sm:w-20 sm:h-20 rounded-full overflow-hidden border-2 border-cyan-500 shadow-cyan-500/60">
          <img
        ...
        {messages.map((msg, index) => (
          <div
            key={index}
            className={`p-3 sm:p-4 my-3 rounded-lg shadow-lg transition-all ${
              msg.sender === "You"
                ? "bg-gradient-to-r from-cyan-600 to-blue-700 self-end ml-auto border border-cyan-400"
                : "bg-gradient-to-r from-pink-600 to-purple-700 self-start border border-pink-400"
            }`}
            style={{
              maxWidth: "75%",
              width: "fit-content",
              fontSize: "0.9rem",
              marginLeft: msg.sender === "You" ? "auto" : "8px",
              marginRight: msg.sender === "You" ? "8px" : "auto",
            }}
          >
            <strong className="text-yellow-300">{msg.sender}: </strong> {msg.text}
            {msg.audio && (
              <audio
                controls
                autoPlay
      ...
      {/* Input Area */}
      <div className="flex flex-wrap items-center gap-3 mt-3 sm:mt-5 p-3 sm:p-4 bg-gray-900/80 rounded-2xl shadow-xl mx-2 sm:mx-12 border border-cyan-700">
        <input
          value={input}
          onChange={(e) => setInput(e.target.value)}
          onKeyDown={handleKeyPress}
          placeholder="💬 Type or speak with your AI..."
          className="flex-1 p-3 rounded-xl bg-gray-800 text-cyan-200 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-cyan-500 placeholder-gray-400"
        />
        <button
          onClick={() => sendMessage(input)}
          className="p-3 bg-gradient-to-r from-pink-600 to-purple-700 hover:from-pink-500 hover:to-purple-600 rounded-xl shadow-md border border-pink-400"
        >
          <Send size={22} />
        </button>
        <button
          onClick={toggleListening}
          className="p-3 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-400 hover:to-emerald-500 rounded-xl shadow-md border border-green-400"
        >
